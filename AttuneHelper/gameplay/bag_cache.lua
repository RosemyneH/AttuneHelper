        ...
        if link then
            ...
            if name and equipLoc and equipLoc ~= "" then
                ...
                if unified then
                    ...
                    local inSet = false
                    if AHSetList then
                        if itemID and legacyKey and AH.AHSetHasGuidScopedEntryForLegacy and AH.AHSetHasGuidScopedEntryForLegacy(legacyKey) then
                            local recSig = AH.GetBagRecGuidSignature and AH.GetBagRecGuidSignature({ bag = bagID, slot = slotID }) or nil
                            if recSig and AHSetList[legacyKey .. "|" .. recSig] ~= nil then
                                inSet = true
                            end
                        else
                            inSet = (AHSetList[idKey] ~= nil or AHSetList[legacyKey] ~= nil or AHSetList[name] ~= nil)
                        end
                    end
